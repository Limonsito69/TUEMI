...
export default function RoutesPage() {
    ...
    const handleSave = async () => {
        if (!name || !schedule || selectedStops.length < 2) {
            toast({ title: "Faltan datos", description: "Verifica el nombre, horario y selecciona al menos 2 paradas.", variant: "destructive" });
            return;
        }
        setLoading(true);
        try {
            ...
            toast({ title: "¡Ruta Creada!", description: "Disponible para asignación." });
            // Reset form
            setName(""); setSelectedStops([]);
        } catch (e) {
            ...
            <Card className="w-1/3 flex flex-col border-none shadow-lg bg-white">
                <div className="p-6 space-y-6 flex-1 overflow-y-auto">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">Une los puntos para crear el recorrido.</p>
                    </div>

                    <div className="space-y-3 bg-slate-50 p-4 rounded border">
                        <div className="grid gap-1">
                            <Label>Nombre</Label>
                            <Input placeholder="Ej: Ruta Sur" value={name} onChange={e => setName(e.target.value)} />
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                             <div>
                                <Label>Horario</Label>
                                <Input placeholder="07:00" value={schedule} onChange={e => setSchedule(e.target.value)} />
                            </div>
                             <div>
                                <Label>Tipo</Label>
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {selectedStops.map((stop, i) => (
                                <div key={i} className="flex items-center gap-3 p-2 bg-white border rounded shadow-sm text-sm">
                                    ...
                                    <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleRemoveStop(i)}><X className="w-3 h-3"/></Button>
                                </div>
                            ))}
                             {selectedStops.length === 0 && <p className="text-xs text-center py-4 text-muted-foreground border-2 border-dashed rounded">Selecciona pines en el mapa.</p>}
                        </div>
                    </div>
                </div>
