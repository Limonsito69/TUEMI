...
import { getSystemStats, getActiveTrips, getDrivers, getRoutes, DashboardStats } from '@/lib/actions';
  ...
  React.useEffect(() => {
    async function loadDashboardData() {
      try {
        ...
        const [statsData, tripsData, driversData, routesData] = await Promise.all([
          getSystemStats(),
          getActiveTrips(),
          getDrivers(),
          getRoutes()
        ...
        <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">vehículos en circulación</p>
            </CardContent>
          </Card>
          
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Usuarios Abonados</CardTitle>
          ...
          <Card className="xl:col-span-2">
            <CardHeader className="flex flex-row items-center">
              <div className="grid gap-2">
                ...
                <CardDescription>Monitoreo de la flota activa en tiempo real.</CardDescription>
              </div>
                    ...
                    recentTrips.map((trip) => {
                        ...
                        <TableRow key={trip.id}>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <AvatarFallback>{driver?.name.charAt(0) || 'C'}</AvatarFallback>
                                </Avatar>
                                <div className="font-medium">{driver?.name || 'Desconocido'}</div>
                            </div>
                            </TableCell>
                            ...
                            <TableCell className="text-right">
                                {trip.passengersAbonado + trip.passengersNoAbonado}
                            </TableCell>
                        </TableRow>
                        );
                    })
